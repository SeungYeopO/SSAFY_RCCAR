...
def main():
    ...
    while (camera.isOpened()):  # 카메라가 Open되어 있다면,
        ...
        blue_mask = cv2.inRange(hsv_img, (80, 110, 110), (120, 255, 255))
        ...
        if len(contours) > 0:
            ...
            # X축의 무게중심을 출력한다.
            teduri = cv2.drawContours(frame, contours, -1, (0, 255, 0), 3)
        ...
        if cv2.waitKey(1) == ord('z'):  # 만약 q라는 키보드값을 읽으면 종료합니다.
            break
...
def server():
    while 1:
        ...
        # 서버에서 모터 속도값 받아오면서 계속 속도 제어


if __name__ == '__main__':
    process1 = Thread(target=main, args=())
    process2 = Thread(target=server, args=())
    process1.start()
    #process2.start()